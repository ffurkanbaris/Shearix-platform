...
func TestCredentialDeliveryFailureIntegration(t *testing.T) {
	...
	app := fiber.New()
	New(repo, internalauth.NewTokenVerifier(token), failing, token, "http://unused").Register(app)
	...
	New(repo, internalauth.NewTokenVerifier(token), blocking, token, "http://unused").Register(concurrentApp)
	...
	New(repo, internalauth.NewTokenVerifier(token), success, token, "http://unused").Register(successApp)
	...
	resetApp := fiber.New()
	New(repo, internalauth.NewTokenVerifier(token), resetFailure, token, "http://unused").Register(resetApp)
