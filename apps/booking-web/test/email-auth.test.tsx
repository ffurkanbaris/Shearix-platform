...
describe("email-only customer authentication contracts", () => {
  ...
  it("registers name and email and shows credential-delivery copy without a password", async () => {
    ...
    expect(mocks.post.mock.calls[0][1]).not.toHaveProperty("phone");
    expect((await screen.findByRole("status")).textContent).toMatch(/sent to your email address/i);
  ...
  it("requests password reset using only normalized email", async () => {
    ...
    await user.type(screen.getByLabelText("Email"), "RESET@EXAMPLE.TEST");
    ...
    expect(mocks.post.mock.calls[0][1]).not.toHaveProperty("phone");
    expect((await screen.findByRole("status")).textContent).toMatch(/temporary password.*email/i);
  });

  it.each([
    ["login", <CustomerLogin />, ["Email", "Password"], "Sign in"],
    ["registration", <Register />, ["Name", "Email"], "Create account"],
    ["reset", <ForgotPassword />, ["Email"], "Send new password"],
  ] as const)("rejects an invalid email in the %s form before API submission", async (_name, component, labels, button) => {
    const user = userEvent.setup();
    render(component);
    for (const label of labels) await user.type(screen.getByLabelText(label), label === "Email" ? "invalid" : "valid-value");
