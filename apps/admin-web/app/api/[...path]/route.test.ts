...
describe("admin Next.js proxy route wiring", () => {
  ...
  it("uses the shared hardened proxy from the actual catch-all route", async () => {
    ...
    const response = await proxyGateway(
      new Request("http://admin.localhost/api/v1/admin/appointments?status=pending", { headers: { cookie: "admin=opaque", "x-user-id": "spoof", "x-tenant-id": "spoof" } }) as NextRequest,
